...
Podstawową decyzją, którą należy podjąć w~trakcie precyzowania założeń dotyczących pierwszego kroku generacji jest organizacja i~format plików źródłowych, na których pracował będzie trzon narzędzia, a~także organizacja plików, które będzie w~stanie wygenerować.
...
Jako że język UML nie spełnia wymagań trzonu generatora w~zakresie elastyczności, domyślnie wspieranym językiem opisu dziedziny aplikacji będzie jeden z~pozostałych trzech opisanych wyżej.
...
Ostatnią decyzją w~ramach założeń dotyczących trzonu generatora jest wybór silnika generacji kodu (ang. \emph{templating engine}) przez niego używanego.
