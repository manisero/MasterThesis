...
\subsubsection{Encje i~ich pola}
...
\begin{itemize}
 \item definicje zdarzeń następują w~opisie dziedziny po linii o~treści ``\verb|Events:|'';
 ...
 \item pojedyncza definicja zdarzenia jednoznacznie określa pojedyncze zdarzenie w~modelu komend systemu;
...
\begin{itemize}
 \item definicje widoków następują w~opisie dziedziny po linii o~treści ``\verb|Views:|'';
 ...
 \item pojedyncza definicja widoku nie określa jednoznacznie pojedynczego widoku w~modelu zapytań systemu:
  \begin{itemize}
   \item określa natomiast treść wyświetlaną na pojedynczym ekranie aplikajcji, co z~biznesowego punktu widzenia jest bardziej intuicyjne;
  \end{itemize}
