...
\subsection{Czy wymagać schematu opisu dziedziny}

\begin{itemize}
 \item że będzie schemat opisu \ref{sec:core:domain_schema_requirement}
 \item ale gdzie ten schemat ma być zapisany
 \item że schema powinna być w jednym miejsu - będzie jako klasa w kodzie
\end{itemize}
...
\begin{itemize}
 \item zdefiniowanie schematu opisu dziedziny na potrzeby CQRS (zapisać każdy w JSONie)
  \begin{itemize}
   \item per tabela (widoki) (duplikacje) (trudność wygenerowania modelu komend, i zdarzeń)
   \item per encja (PresentIn) (brak duplikacji) (encje generują się same, generacja widoków i zdarzeń wymaga trochę pracy, ale jest ok)
  \end{itemize}
\end{itemize}
